...
<?php
            if(!empty($_POST)){
                ...
                if(empty($errors)){
                    ...
                    $modifier = $connexion->prepare('UPDATE shop SET shop_address = :shop_address, shop_mail = :shop_mail, shop_phone = :shop_phone WHERE id = :id');
                    $modifier->bindValue(':id', htmlspecialchars($_POST['id']));
                    $modifier->bindValue(':shop_address', htmlspecialchars($_POST['title']));
                    $modifier->bindValue(':shop_mail', htmlspecialchars($_POST['author']));
                    $modifier->bindValue(':shop_phone', htmlspecialchars($_POST['content']));
                    $modifier->execute();
?>
                    <h3 class="alert alert-success col-lg-2 m-auto text-center">
                        Coordonnées modifié !
                    </h3>
<?php
                }else{
